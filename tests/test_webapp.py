...
def test_create_func(json_create: dict, valid_json: tuple) -> None:
    ...
    """
    requests.post(
        'http://localhost:5001/inner/meteodata/create', json=json_create, timeout=TOUT,
    )
    response = requests.get('http://localhost:5001/inner/meteodata/get', timeout=TOUT).json()
...
def test_delete_func(status_code: int) -> None:
    ...
    response_code = requests.delete(
        'http://127.0.0.1:5001/inner/meteodata/delete',
        json={'id': response_id},
        timeout=TOUT,
        ).status_code
